...
export default function Pagination(props: any) {
    const { currentPage, totalPages, totalItems, pageSize, onPageChange, onPageSizeChange } = props;
            ...
            <div className="flex justify-between items-center">
                <div className="flex items-center space-x-6 hidden lg:flex">
                    ...
                    <div className="flex items-center space-x-1">
                        ...
                        <select name="row-count" id="row-count"
                            className="px-3 py-1 border border-[#e5e7eb] rounded-sm outline-none bg-white  hover:border-[#667eea] focus:border-#667eea] transition-colors cursor-pointer"
                            value={pageSize}
                ...
                <div className="flex  flex-1 items-center justify-center lg:justify-end space-x-2">
                    ...
                    {getPageNumbers().map((page: any, index: number) => (
                        page === "..." ? (
                            <span key={`ellipsis-${index}`} className="py-8 text-[#9ca3af] font-semibold">
                                ...
                            </span>
                        ) : (
                            <button
                                key={page}
                                className={`${pageBtnClass} ${currentPage === page ? "bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white border-[#667eea] font-semibold" : ""}`}
                                onClick={() => {
                                    onPageChange(page);
                                    handlePageChange()
                                }}
